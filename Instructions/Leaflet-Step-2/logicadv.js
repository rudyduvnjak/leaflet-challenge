...
function createFeatures(earthquakeData) {
    ...
    d3.json(queryUrl2).then(function(response) {
        ...
        var tectonicFeatures = response.features; 
        var plateData = L.geoJSON(tectonicFeatures, {
            color: "magenta"
        });
        //Send earthquake and tectonic plateData to the createMap function
...
function createMap(earthquakes, plateData) {
    ...
    // Define starting center of the map
    // Define map zoom
    // Define layers initially displayed
    var myMap = L.map("map", {
        center: [
            37.09, -95.71
        ],
        zoom: 4,
        layers: [streetMap, earthquakes, plateData]
    });
